...
public class CameraActivityFragment extends Fragment
    ...
    private void setEnabledLinearLayout(LinearLayout ll, boolean b) {
        for ( int i = 0; i < ll.getChildCount();  i++ ){
            ...
            childView.setEnabled(b); // Or whatever you want to do with the view.
        }
    }
    ...
    long mExposureTimeTest = 50000 *MICRO_SECOND;
    long numImagesTest = 8;
    long gap = 1000;
    @Override
    ...
    private void setExposureAppropriately(CaptureRequest.Builder builder) {
        if (isAutoExposure()) {
            ...
            builder.set(CaptureRequest.SENSOR_EXPOSURE_TIME, mExposureTimeTest);
    ...
    private void takePicture() {
        ...
        try {
            timingTest = new FileWriter(new File(getActivity().getExternalFilesDir(null), "timing.txt"));
        }  catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    CameraCaptureSession.CaptureCallback CaptureCallback
        ...
        public void onCaptureStarted (CameraCaptureSession session, CaptureRequest request, long timestamp, long frameNumber) {
            Log.d(TAG, "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx: " + (System.nanoTime() - lastTime));
        }
        @Override
        public void onCaptureCompleted(@NonNull CameraCaptureSession session,
                                       @NonNull CaptureRequest request,
            ...
            try {
                timingTest.write(mExposureTimeTest + ", " + result.get(CaptureResult.SENSOR_EXPOSURE_TIME) + "\n");
            } catch (IOException e) {
                e.printStackTrace();
            }
            if (isBurstMode()) {
                ...
                image.close();
                if (imageNum == numImagesTest) {
                    imageNum = 0;
                    unlockFocus();
                    try {
                        timingTest.flush();
                        timingTest.close();
                        getActivity().sendBroadcast(new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE, Uri.fromFile(new File(getActivity().getExternalFilesDir(null), "timing.txt"))));
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                    return;
